...
public class ServerTable extends JPanel {
	...
	protected void decorateTable() {
		infoTable.setFont(new Font("幼圆", Font.PLAIN, 20));	// 设置表格内容字体
		infoTable.setForeground(Color.WHITE);	// 表格前景色
		infoTable.setSelectionForeground(Color.BLACK);	// 选择文本的前景色
		...
		infoTable.setGridColor(Color.LIGHT_GRAY);	// 表格边框的颜色
		...
		header.setBackground(new Color(0x22355F));	// 设置头部背景色
	...
	class HeaderCellRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
			...
			label.setForeground(Color.WHITE);	// 表头前景色
			return label;
		}
	...
	protected void initScrollPane() {
		...
		scrollPane.setSize(600,565); // 和panel一样大
